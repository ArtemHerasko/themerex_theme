...
<script type="text/javascript">
	jQuery(document).ready(function($){
		...
		if( $('<?php echo $this->selector; ?>').length != 0 ) {
			$('<?php echo $this->selector; ?> span.<?php echo $this->slug; ?>').parents('<?php echo $this->selector; ?>:first').remove();
		} else {
			$('h1 span.<?php echo $this->slug; ?>').parents('h1:first').remove();
			$('h2 span.<?php echo $this->slug; ?>').parents('h2:first').remove();
		}
